...
class Weekly {
  ...
  generatePdfDocumentFromContent() {
    ...
    doDebugLog("generatePdfDocumentFromContent", "this.pdfDocument", this.pdfDocument);
  }

  buildPdfContent() {
    var departments = Object.keys(this.departmentNames);
    doDebugLog("buildPdfContent", "departments", this.departmentNames);

    for (var page = 1; page < 8; page++) {
      doDebugLog("buildPdfContent", "page loop " + page, "loop started");
      if (page == 1) {
      ...
      for (var deptIdx = 0; deptIdx < departments.length; deptIdx++) {
        if (this.departmentNames[departments[deptIdx]] == "none") {
          doDebugLog("buildPdfContent", "deptIdx loop check for none " + deptIdx, this.departmentNames[departments[deptIdx]]);
          continue;
        }
        ...
        doDebugLog("buildPdfContent", "right before employeeSrcData " + deptIdx, this.departmentNames[departments[deptIdx]]);
        if (this.orgData[page][departments[deptIdx]] === undefined || typeof this.orgData[page][departments[deptIdx]] === "undefined") {
          doDebugLog("buildPdfContent", "skipping", "");
        } else {
          let employeeSrcData = this.orgData[page][departments[deptIdx]];
          doDebugLog("buildPdfContent", "employeeSrcData " + deptIdx, employeeSrcData);
      ...
      doDebugLog("buildPdfContent", "page loop " + page, "loop finished");
    }
  }
  ...
  async openFileAsync() {
    ...
    doDebugLog("openFileAsync", "function ok", "");
  }

  async getSheetNames() {
    this.sheets = this.workbook.SheetNames;
    doDebugLog("getSheetNames", "this.sheets", this.sheets);
  }
  ...
  async getFormattedDataFromSheets() {
    for (var sheetCount = 0; sheetCount < this.sheets.length; sheetCount++) {
      ...
      doDebugLog("getFormattedDataFromSheets", "sheetData loop " + sheetCount, sheetData);
      doDebugLog("getFormattedDataFromSheets", "schedulesStartAtRow loop " + sheetCount, schedulesStartAtRow);
      this.getEmployeeScheduleLines(sheetData, schedulesStartAtRow);
      //doDebugLog("getFormattedDataFromSheets", "this.orgData loop " + sheetCount, this.orgData);        
    }
    doDebugLog("getFormattedDataFromSheets", "loop finished", "");
  }
  ...
  async sortOrgDataByEmployeeShift() {
    ...
    doDebugLog("sortOrgDataByEmployeeShift", "this.orgData", this.orgData);
  }
